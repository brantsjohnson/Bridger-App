...
| 2026-09-14 | Open queue items also mirrored as GitHub issues with agent:claude/agent:cursor/agent:grokbot/needs:brant labels, so Cursor and Grokbot have a shared board even before cloud-agent auto-pickup is confirmed | ops/queue/ is only visible to whoever has the repo open; Issues are visible from any GitHub-aware tool immediately | Claude |
