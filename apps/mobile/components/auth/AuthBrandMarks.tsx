// ============================================
// WHAT THIS FILE DOES (plain English):
// The official Google "G" and Apple marks for the Sign in screen. They sit to
// the left of "Continue with Google" and "Continue with Apple" so the buttons
// look like the real brand sign-in paths, not plain text rows.
//
// BRAND RULE: keep Google's four official colors on the G mark. The Apple mark
// follows the button label color (dark on light, light on dark).
// ============================================
import React from 'react';
import Svg, { Path } from 'react-native-svg';
import { useThemeColors } from '@bridger/ui';

/**
 * Google's multicolor G mark. Same palette Google uses on Sign in with Google.
 */
export function GoogleMark({ size = 20 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" accessibilityElementsHidden>
      <Path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <Path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <Path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <Path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </Svg>
  );
}

/**
 * Apple mark for Sign in with Apple. Matches the button label in light/dark mode.
 */
export function AppleSignInMark({ size = 20 }: { size?: number }) {
  const c = useThemeColors();

  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" accessibilityElementsHidden>
      <Path
        fill={c.ink}
        d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.208-1.23-3.234-4.882-.026-3.053 2.494-4.52 2.61-4.599-1.442-2.129-3.675-2.366-4.437-2.405-1.585-.13-3.31.9-4.062.9zm3.378-3.066c.72-.868 1.207-2.076 1.075-3.276-1.06.043-2.34.708-3.128 1.594-.707.784-1.324 2.038-1.157 3.213 1.183.092 2.394-.6 3.21-1.53z"
      />
    </Svg>
  );
}
