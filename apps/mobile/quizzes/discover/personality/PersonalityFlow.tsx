...
export function PersonalityFlow({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const draft = await loadDiscoverDraft<PersonalityAnswer>(SLUG);
      if (cancelled) return;
      if (draft && draft.phase !== 'result' && (draft.answers?.length || draft.phase === 'take')) {
        startedAt.current = draft.startedAt || Date.now();
        lastStep.current = PERSONALITY_QUESTIONS[draft.index]?.id ?? 'take';
        setPhase(draft.phase === 'intro' ? 'take' : draft.phase);
        setIndex(Math.min(draft.index, PERSONALITY_QUESTIONS.length - 1));
