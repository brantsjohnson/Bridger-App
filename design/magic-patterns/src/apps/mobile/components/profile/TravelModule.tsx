import React from 'react';
import { AnimatePresence, motion } from 'framer-motion';
import { cn } from '../../../../packages/ui';
import { PLACES } from '../../state/mock-data';
import countries from '../../assets/geo/countries-110m.json';

const MAP_VB = { w: 360, h: 180 };
const CORAL = '#FF5A1F';

type Place = (typeof PLACES)[number];

type CountryFeature = {
  properties: { ISO_A2: string; NAME: string };
  geometry:
    | { type: 'Polygon'; coordinates: number[][][] }
    | { type: 'MultiPolygon'; coordinates: number[][][][] };
};

function project(lng: number, lat: number) {
  return {
    x: ((lng + 180) / 360) * MAP_VB.w,
    y: ((90 - lat) / 180) * MAP_VB.h
  };
}

function geometryToPath(
  geometry: CountryFeature['geometry']
): string {
  const rings =
    geometry.type === 'Polygon'
      ? geometry.coordinates
      : geometry.coordinates.flatMap((p) => p);
  return rings
    .map((ring) => {
      const pts = ring.map(([lng, lat]) => project(lng, lat));
      const [first, ...rest] = pts;
      return (
        `M${first.x.toFixed(2)} ${first.y.toFixed(2)}` +
        rest.map((p) => `L${p.x.toFixed(2)} ${p.y.toFixed(2)}`).join('') +
        'Z'
      );
    })
    .join('');
}

const COUNTRY_PATHS = (countries as { features: CountryFeature[] }).features.map((f) => ({
  code: f.properties.ISO_A2,
  d: geometryToPath(f.geometry)
}));

/**
 * Two views in one contained module, the same idiom as the hobbies widget.
 * Page 1 is a real stylized world map with country fills + pins; swipe for list.
 */
export function TravelModule() {
  const [page, setPage] = React.useState(0);
  const [active, setActive] = React.useState<string | null>(null);
  const place = PLACES.find((p) => p.id === active);
  const tagged = new Set(
    PLACES.map((p) => ('countryCode' in p ? (p as Place & { countryCode: string }).countryCode : ''))
      .filter(Boolean)
  );

  return (
    <div>
      <motion.div
        drag="x"
        dragConstraints={{ left: 0, right: 0 }}
        dragElastic={0.12}
        onDragEnd={(_e, info: { offset: { x: number; y: number } }) => {
          if (info.offset.x < -50) setPage(1);
          if (info.offset.x > 50) setPage(0);
        }}
        className="touch-pan-y">
        <AnimatePresence mode="wait" initial={false}>
          {page === 0 ? (
            <motion.div
              key="map"
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: -20 }}
              transition={{ duration: 0.2 }}>
              <div className="relative overflow-hidden rounded-card border border-ink-line bg-surface text-ink">
                <svg
                  viewBox={`0 0 ${MAP_VB.w} ${MAP_VB.h}`}
                  className="h-[168px] w-full"
                  role="img"
                  aria-label="Places traveled">
                  {COUNTRY_PATHS.map((c) => {
                    const on = tagged.has(c.code);
                    return (
                      <path
                        key={c.code}
                        d={c.d}
                        fill={on ? CORAL : 'currentColor'}
                        fillOpacity={on ? 0.35 : 0.06}
                        stroke="currentColor"
                        strokeOpacity={0.28}
                        strokeWidth={0.35}
                      />
                    );
                  })}
                </svg>

                {PLACES.map((p) => {
                  const lat = 'lat' in p ? (p as Place & { lat: number }).lat : 0;
                  const lng = 'lng' in p ? (p as Place & { lng: number }).lng : 0;
                  const { x, y } = project(lng, lat);
                  return (
                    <button
                      key={p.id}
                      type="button"
                      onClick={() => setActive((v) => (v === p.id ? null : p.id))}
                      aria-label={p.label}
                      aria-pressed={active === p.id}
                      style={{
                        left: `${(x / MAP_VB.w) * 100}%`,
                        top: `${(y / MAP_VB.h) * 100}%`
                      }}
                      className={cn(
                        'absolute -translate-x-1/2 -translate-y-full transition-transform',
                        active === p.id && 'scale-110'
                      )}>
                      <span className="block h-3 w-3 rounded-full border-2 border-ink bg-coral" />
                    </button>
                  );
                })}
              </div>

              <div className="mt-2.5 min-h-[34px]">
                {place ? (
                  <p className="text-[13px] font-semibold text-ink">
                    <span className="font-bold">{place.label}</span> · {place.note}
                  </p>
                ) : (
                  <p className="text-[12px] font-medium text-ink-mute">
                    {PLACES.length} places · tap a pin
                  </p>
                )}
              </div>
            </motion.div>
          ) : (
            <motion.div
              key="list"
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: 20 }}
              transition={{ duration: 0.2 }}
              className="no-scrollbar max-h-[204px] space-y-2 overflow-y-auto pr-0.5">
              {PLACES.map((p) => (
                <div
                  key={p.id}
                  className="flex items-center gap-3 rounded-card border border-ink-line bg-surface px-3.5 py-3">
                  <span
                    aria-hidden="true"
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-coral/25 text-[16px]">
                    {p.emoji}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[13px] font-bold text-ink">{p.label}</span>
                    <span className="block truncate text-[12px] font-semibold text-ink-mute">
                      {p.note}
                    </span>
                  </span>
                  {p.year && (
                    <span className="shrink-0 text-[11px] font-bold text-ink-mute">{p.year}</span>
                  )}
                </div>
              ))}
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>

      <div className="mt-3 flex items-center justify-center gap-1.5">
        {['Map', 'List'].map((label, i) => (
          <button
            key={label}
            type="button"
            onClick={() => setPage(i)}
            aria-label={label}
            aria-current={page === i}
            className={cn(
              'h-1.5 rounded-full transition-all',
              page === i ? 'w-5 bg-ink' : 'w-1.5 bg-ink/20'
            )}
          />
        ))}
      </div>
    </div>
  );
}
