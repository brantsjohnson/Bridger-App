# Scrapbooks: phase plan and accountability log

This is the working checklist for turning Updates into Scrapbooks. Each phase lists what ships, the files it touches, how it is verified, and what "done" means. Check boxes are ticked in the same change that lands the work. Branch: `feat/scrapbooks` (kept off `main` until the founder has used Phase 1 and wants to keep it).

Founder decisions recorded 2026-09-08:
- Camera roll is allowed for scrapbook pages. Circle replies and stickers stay capture-only.
- User-facing name is **Scrapbook** (tile: Your scrapbook, profile tab: Scrapbook). Code stays `stories`.
...
- [x] Labels: Your scrapbook, Scrapbooks (Home title), Scrapbook (profile tab), Start your scrapbook, replies to your scrapbook.
...
- [ ] Magic Patterns: design `ScrapbookPage`, `LayoutCarousel`, `CountPill`, `ComposerBar`, `CaptureRail`, `PromptsTray`, `CaptionSheet`, `AudienceSheet`, `CustomizeTray` in the library, then reconcile the RN components to them.
...
- [ ] Caption sheet gains **Record** (20 s, mic in context, waveform + timer). Audio saved as `media.kind = 'audio'`; server transcribes (stories module already owns video transcription); transcript lands in the caption text, editable. `VoiceData.displayMode`.
- [ ] `voice` element: `▶ 0:16` chip placeable on the page.
...
- [ ] `person` element: pick friends (manual, event attendees, existing tags); `displayStyle avatars | names | avatars_and_names`. Stored as user ids, never names in analytics.
...
- [ ] PRIVACY.md: audio captions stored + transcribed server-side; people tags; coarse places. NOTIFICATIONS.md: tagging a person is a new notify-able outcome (destination: the page).
...
- [ ] Drag, resize, rotate elements (gesture-handler + reanimated), 44 pt handles, `userModified = true` on touch, snap guides, Reduce Motion aware.
...
- [ ] Text elements: fonts (sans / pixel / hand), sizes, colors; Dynamic Type friendly.
- [ ] Decor library v1: frames (Polaroid, thin, film strip, torn), tape, paper, stickers, doodles, backgrounds (solid + paper presets). Bridger-hosted assets only.
- [ ] `clipping` element (text or image).
- [ ] Undo / redo across gestures.
...
- [ ] Cutouts (on-device subject lift where the OS offers it; otherwise manual lasso). Source photo untouched.
