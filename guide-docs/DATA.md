...
- **Encryption at rest (roadmap).** Zone A contact fields, pending people phones, and message bodies move to **application ciphertext** with **split-key custody** (Supabase holds blobs; AWS KMS/Secrets Manager holds DEKs; phone friend-find uses an HMAC key in a **second** secret). Phase 1b adds per-tier and per-custom-group DEKs plus group-share keys; Phase 2 adds client-held Close keys and true E2E DMs. Full contract: `docs/ENCRYPTION-AND-ACCESS.md`. This pass is docs only; no production cutover yet.
...
- [ ] Breach-ready encryption (`docs/ENCRYPTION-AND-ACCESS.md`): split keys, Phase 1 PII/messages ciphertext, rotation so old dumps die, no plaintext PII in logs/analytics/payloads; Phase 1b tier/group DEKs; Phase 2 E2E DMs and PSI-style in-common (implementation tracked separately).
