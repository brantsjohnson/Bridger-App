...
│   └── out-of-scope.md
│
├── platform/                     ← data rails: discovery + plan (privacy first)
│   ├── PROCESS.md
│   ├── 00-DATA-INVENTORY.md
│   ├── 01-ARCHITECTURE.md
│   ├── 02-DATA-CATALOG-SPEC.md
│   ├── 03-POLICY-AND-SECURITY.md
│   ├── 04-EXTENSION-MANIFEST-SPEC.md
│   ├── 05-CONNECTION-CONTRACTS.md
│   ├── 06-IMPLEMENTATION-PLAN.md
│   ├── 07-STANDING-RULE.md
│   ├── 08-OPEN-QUESTIONS.md
│   └── inventory/                ← generated schema, policies, API map, catalog seed
│
├── adr/                          ← decision records (proposed until the founder accepts)
│   ├── 0001-policy-engine-in-nest.md
│   ├── 0002-declarative-extensions.md
│   ├── 0003-catalog-json-in-repo.md
│   └── 0004-consent-receipts-and-omit-denied.md
│
...
├── aws-agent-rules.mdc
└── data-catalog.mdc           ← catalog update in the same PR as a data change
```
...
| `PERSONAL-DATA.md` | [BUILDING] | Personal Vault + Personalize. Settings preview is live (coming soon, collects nothing). Processing flags beyond matchable, My Data, custom matchers, and bring-your-own datastore are later. |
...
### 4.8 · Platform data rails (`platform/` — plan written, not implemented)

Privacy wins over extensibility. Visual design is out of scope. Follow `platform/PROCESS.md`. Next implementation step is Phase 1 in `06-IMPLEMENTATION-PLAN.md` (catalog file and CI). It does not change the running app. Founder gates are in `08-OPEN-QUESTIONS.md`.

| Doc | Status | Role |
|---|---|---|
| `platform/PROCESS.md` | [BUILDING] | Working brief for this effort. |
| `platform/00-DATA-INVENTORY.md` | [BUILDING] | Phase 0 findings. Real tables, policies, routes, consent, gaps. |
| `platform/inventory/` | [BUILDING] | Generated appendix. Regenerated by `node scripts/data-inventory/generate.mjs`. |
| `platform/01-ARCHITECTURE.md` | [BUILDING] | Target rails: Nest policy layer, RLS as the second fence, declarative extensions. |
| `platform/02-DATA-CATALOG-SPEC.md` | [BUILDING] | Catalog entry schema, purposes, consent basis. |
| `platform/03-POLICY-AND-SECURITY.md` | [BUILDING] | Deny by default, omit denied fields, threat model, k = 10. |
| `platform/04-EXTENSION-MANIFEST-SPEC.md` | [BUILDING] | Manifest, review, install, messages dogfood. |
| `platform/05-CONNECTION-CONTRACTS.md` | [BUILDING] | Post modality vs receive preference. Receive never grants access. |
| `platform/06-IMPLEMENTATION-PLAN.md` | [BUILDING] | Phases, TestFlight safety, effort, founder gates. |
| `platform/07-STANDING-RULE.md` | [BUILDING] | CI, nightly drift, PR checklist. |
| `platform/08-OPEN-QUESTIONS.md` | [BUILDING] | Choices that still need Brant. |
| `adr/0001` through `0004` | [BUILDING] | Proposed decisions: policy location, declarative extensions, catalog in git, receipts and omit. |
...
| `data-catalog.mdc` | [RULE] | Same PR updates the data catalog when tables, policies, API fields, analytics, or extension capabilities change. Classify up if unsure. |
...
- **Two independent per-attribute consents:** `visibleToTier` (who sees it) and `matchable` (may Discover use it). A field can be friend-visible but not matchable. **Identity/beliefs are never bulk-matchable.** Personalize and future matchers cannot widen someone else's eligibility or visibility (`PERSONAL-DATA.md`).
...
*Last rebuilt: 2026-09-21. Data rails plan written under `platform/` and `adr/` (not implemented). Cursor rule `data-catalog.mdc`. Phase 0 inventory remains the factual baseline.*
