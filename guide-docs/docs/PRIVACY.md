...
- Sign-in via **phone number + SMS one-time code** (default). The number is stored in E.164 on `user_contacts` and is the account merge key. **Continue with Google** / **Sign in with Apple** / email remain available when `EXPO_PUBLIC_AUTH_MODE=legacy`. First use creates the account; later visits sign in.
