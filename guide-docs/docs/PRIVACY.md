...
- **Social battery (onboarding step):** an optional number, 0 to 7+, of nights out you like in a normal week. Stored on `user_settings.social_battery`. **Why:** to pace how often Bridger nudges *you* toward plans. **Who sees it:** only your own pacing logic, never shown to others as a number. Skippable.
...
- **New onboarding "taste" facts:** current job, dream job, the song on repeat (typed into canonical `currently_song`, or overridden by a linked music Listening pick), hometown / current town (About Me rows: towns only, never a street address), favorite place visited (**place name only**; we may **coarsely geocode** it via Photon/OSM to place a pin on your travel map with a FAV flag and approx lat/lng, never a street address), and a weekly recap highlight as a **20 second voice memo** (no typing option on the onboarding recap screen). Each is optional/skippable, tagged with its own **visibility** on the Privacy & Control screen (Close / Friends / Acquaintances), and hard-deleted with the account. Geocode query text is never logged to analytics. The recap voice memo is your own capture; analytics record only that a recap was added and its length, never the audio or words.
