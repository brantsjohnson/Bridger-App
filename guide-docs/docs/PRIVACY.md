...
- This draft covers the mobile apps (iOS / Android), the web build, the NestJS API, and Supabase-backed storage. The public pages people and carriers should open are `https://bridger.social/privacy.html` and `https://bridger.social/terms.html`.
...
- **Capture-only media**, except: the **profile photo**; photos/short videos you pick from the camera roll for a Collage page; **one photo on an Inside Joke** (co-op members); the **event cover**; and **event album uploads** for a guest-list event.
- **No vanity metrics** (no follower counts, view counts, invited totals, streaks, public leaderboards). Private owner-only numbers are allowed (host headcount vs cap; an Influencer's own Circle size; author-only quiz-finish count).
...
- Sign-in via **phone number + SMS one-time code** (default). You pick a **country dial code** (defaults from your device region) and type the local number; we store the result in **E.164** on `user_contacts` and use it as the account merge key. Analytics may record the 2-letter country (`country_iso`) only, never the digits. Entering your number and tapping **Send me a code** is consent to receive that one sign-in text. Sign in shows SMS disclosures (frequency, Msg & data rates, Reply HELP / STOP) plus links to Privacy and Terms. **Continue with Google** / **Sign in with Apple** / email remain available when `EXPO_PUBLIC_AUTH_MODE=legacy`. First use creates the account; later visits sign in. The session stays on this device so you are not asked for a new text every time you open the app.
- **SMS program (Bridger Sign-in Texts):** Twilio delivers sign-in codes via Supabase Auth. Frequency is only when you request a code. Not marketing. Public CTA mirror for carrier review: `https://bridger.social/sms-opt-in.html`. **We do not sell or share your SMS opt-in data or personal information with third parties for marketing purposes.** Reply **HELP** for help (`hello@bridger.social`); reply **STOP** to opt out. Carriers are not liable for delayed or undelivered messages.
- **Optional Face ID / fingerprint unlock:** in Settings you may turn on a lock that uses the phone's own Face ID, Touch ID, or fingerprint to open Bridger. **Why:** so you can protect a saved session without paying for another SMS code. **Who sees it:** only this device. Bridger never receives or stores your face or fingerprint. The on/off choice is a yes/no flag on this phone, keyed to your account. **How to stop:** turn the toggle off, or sign out. Off by default. If Face ID fails you can use the phone passcode, or sign out and use a new text. Not offered on web.
- **Pending people (private cards):** when you tap **Connect your contacts** on Friends (Add friend or the empty state) and pick one person, we store that one phone (E.164) plus the name you saw on an author-only `pending_people` row. You can write private notes on the card you made (`pending/[id]`). We never upload your whole address book. **Who sees it:** only you. **How to stop:** delete the note / account. When they join with that number, the card is replaced by their real profile; your notes stay private to you and they do not see them.
...
- **Store disclosures:** Apple Privacy Manifest (`PrivacyInfo.xcprivacy`) and Play Data safety list **Phone Number** as account-linked, used for app functionality, not tracking. Product analytics (PostHog) is listed the same way.
- **Onboarding preference chips:** New onboarding may save opaque keys for membership interests and what would help (`user_settings.membership_interests` / `help_interests`). Help keys are feature ids such as `plans` or `keep-up`, never free-text or names. Older drafts may still hold `page_authoring`.
...
- **Profile photo:** the one upload exception. Stored as `media` + `user_identity.avatar_media_id`. The app loads it via short-lived signed URLs (`avatarUrl` on `/me`, `/me/profile`, and friend cards). The **About me** card on Profile uses this same photo by default; while Edit is on, tapping that picture opens Take / Upload and updates the same avatar everywhere (header + About me). Hard-deleted with the account.
- **Profile photo "looks" (filters):** you can style your profile photo with a look (**Pop art**, **Comic**, **X-ray**, or **Sepia**). While you pick, all four looks **preview 100% on your phone** (Pop art as a Warhol grid; Comic / Sepia / X-ray as an on-device color matrix). A badge says **Ran 100% locally - not AI**. The saved avatar is then **baked on our own server**: the phone uploads your normal photo to your private `media` bucket, then `POST /photo-filters/apply` has the NestJS API read that photo, repaint it with **ImageMagick** (a local image tool, not an outside company and not any AI model), and save the finished picture back as a new owned `media` row. The original stays on file (`user_identity.avatar_original_media_id`); your avatar points at the filtered copy so every face in the app renders that look fast. Which look you picked is stored as `user_identity.avatar_filter`. You can change the look later from Profile **Edit** → Photo look (re-bakes from the original; no re-upload required). **Why:** those looks need image operations that should not re-run on every Avatar render. **Who sees it:** only you until you tap Continue (or Save look), then it becomes your avatar under the normal tier rules. The photo is never sent to any AI/model or third party for this; the work happens inside our own container. Both the original and the filtered copy are hard-deleted with the account.
...
- You may **link Spotify or Apple Music** from Settings (or during Old onboarding). This is **account linking**, not “sign in with Spotify / Apple Music.” Bridger login stays phone OTP (or Google / Apple when legacy mode is on).
...
- **Updates, shown as Collage pages** (2026-09-08, camera-first 2026-09-09). A post is one 8.5 x 11 page: up to 4 photos/videos a day across 1 to 4 pages, typed words, a date stamp, optional voice notes, friend tags, and cut-outs, each stored as an "element" with its position (fractions of the page) in `scrapbook_pages` / `scrapbook_elements`. **Media may be captured live in Bridger or chosen from the camera roll** (the roll is read only for the pictures you pick; we never scan your library). Each photo remembers where it came from (`bridger_camera` / `camera_roll` / `event` / `shared`). You can save a single photo or a finished page to your camera roll (add-only permission, asked on that tap). A flattened preview image of the page is stored alongside so the Home tile and Catch-Up can show it. Pages can be edited the same day (a `revision` number goes up; friends see the ring light again on their own phone; no view counts). Audience per page: **Only me** (nobody else can read it, enforced by row-level security), Close, Friends, Everyone, or a co-op group. Same retention as before: ~24 h live, ~30 days for free members, forever for co-op; hard-deleted with the account. Photos are never sent to AI. Subject lift (cut-out) runs on the phone when available (Apple Vision on iOS, Google on-device subject tools on Android). We do not upload a photo to cut it out. If those tools are missing, you pick a simple shape instead. **Voice notes** are stored as audio you recorded; optional transcription runs server-side and the words may join the day summary (your words only). **Friend tags** store that friend's id on the page; they may get a "tagged you" alert. Analytics never logs captions, transcripts, or names. Circle video replies and stickers stay capture-only. The unposted draft lives only on your phone and is cleared when you post.
...
- **Inside Jokes** (`quips` / `quip_tags`): the text you type, who you tagged as saying it, an optional event, the note color, and (co-op only) one photo (`photo_media_id` → your private `media` row). **Why:** so the joke can show on Friends (newest first) and on your wall plus the tagged person's wall. **Who sees it:** you, the person you tagged, and anyone else you tag. Joke text is never sent to analytics or to AI. The photo is asked for in context (camera or one picked camera-roll item); we never scan your library. Hard-deleted with the joke or the account.
- Poll questions/votes, Touch Grass signals (audience + when + why), event details, recap voice answers, Friend Pod suggested questions + votes (question text is stored so friends can vote; never sent to analytics or to the leftover-prompt model), co-op portal ideas/comments (shown as "A member," no person names on the member portal).
- **Friend Pod week lock:** each Monday the live 5 prompts are saved on `recap_weeks` / `recap_questions`. Auto weeks use rose / thorn / bud (Bridger copy), the most-voted unused suggestions, and short fill-ins. An optional model call may write leftover fill-ins from Bridger copy only (no names, no friend-typed questions). If that job is off, a canned bank is used. Opening Friend Pod never waits on a model. You pick Close / Friends / Acquaintances when you post your recap.
...
- **Event host notes (planned):** a host or co-host may send a short text note plus an optional photo to people going and/or invited. Guests see it on the event page and may get a notification. It is not a chat and does not create a Messages thread. Hard-deleted with the event or account. Analytics never logs the note text.
- **Event photo album (planned):** guests may add photos (live capture or camera roll, this event only) to a shared album. Guest-list only (not shared-link outsiders). You can **Save** a photo to your device library (add-only permission; we never scan your camera roll for this). Save is remembered per person per photo. Album access for a photo you have not saved expires **7 days after the event ends**. The author's own Update on their profile still follows personal retention (Free Lite ~30 days / co-op keep). The album has its own per-event storage cap; the host may pay for a larger pool. Hard-deleted with the event or account.
- **What version of me quizzes (planned):** if you build a personality quiz about yourself, we store versions (labels, blurbs, photos), questions, answer weights, and friends' results. Friends take it from your share link. Who-got-who is visible to you and to mutual friends who took it, not as a public leaderboard. Questions, labels, and photos are UGC. Results do **not** feed Discover matching. Hard-deleted with the account or when you delete the quiz.
- **Quiz result + share/referral (Which "J" name are you?):** `jname_results` stores your **first** finished result only (a J-name, a percent, and your top J-name picks by score). That first result is the one friends see, the one used for the compatibility %, and the one kept on your Profile after Home rotates the featured quiz. A `jname_shares` row is one stable share link per person, snapshotting that first J-name/percent so the free web page can render it. A friend can **take the quiz with no Bridger account**. Their first result stays on that device until they make an account; signup uploads that first result (not later fun retakes) and **adds the sharer as a friend** so both can see the duo / compatibility result. `jname_referrals` records that someone opened your link (logged-out opens use only an opaque device id, `anon_ref`, never a name) and is resolved on signup. The public view (`/q/<token>`) and the take (`/quiz/what-j-name`) need no account; the "your version of X" friend board requires an account (API `GET /jname/leaderboard`). When both you and a friend have finished their first result, we also show a fun pairwise **compatibility %** derived only from those first results (same J-name scores high; a top-pick hit scores medium-high; otherwise closeness of the two J-% numbers). Never answers or explanations. Notifications: `jname_link_opened` when someone opens your link; `jname_top_match` when a friend lands on a J-name in your top 3 picks; `connection_accepted` when the quiz-share signup adds the friend. All of these hard-delete with the account.
- **In-progress J-name draft (on-device only):** if you leave mid-take, Bridger may keep your answer picks on this phone (AsyncStorage) for about 7 days so you can resume instead of starting over. That draft never goes to our servers or to analytics. Finishing the quiz or starting a retake clears it.
- **J-name fun retake (on-device only):** after your first result is saved, you may retake the quiz for fun. The latest fun result stays on this phone (AsyncStorage) so you can flip between Your result and Fun retake. It is never uploaded, never used for friend compatibility, never written to `jname_results` or the share snapshot, and never sent to analytics as content (we may record that a retake finished, not which name you got). Clearing the app's local storage removes it.
- **J-name guest take (on-device only):** if you finish Which J name before you have an account, that first result (and the share token of the friend who invited you) stays on this device until you sign up. Signup uploads the first result and may add that friend. Fun retakes are not uploaded. Clearing the app's local storage removes the guest stash.
- **Discover Connect Over quiz drafts (on-device only):** Your Funny Bone, Your Vibe, What Gets You Going, and The Friend Zone may keep mid-take picks on this phone (~14 days) so you can Save and exit and come back (cards can show a % finished). Never uploaded; Discard and exit or finishing clears them. Behind the Scenes stays one-shot / no mid-take draft.
- **Last-seen tab pictures (on-device only):** so Home, Friends, Events, Discover, Messages, and Profile do not rebuild from an empty box every time you tap a tab, the phone keeps the last layout and list it already showed (AsyncStorage, keyed to your account or demo). New items still refresh in the background. This is a speed cache, not a second copy of the server. Sign-out, leaving demo, or clearing the app's local storage removes it. Account deletion also clears it because sign-out runs first. Never sent to analytics as content.
...
- **Circles (planned):** a separate edge from friends. When you add an Influencer you choose a visibility tier (Acquaintance / Friend / Close) that controls which of your profile facts they may see, same as those friend tiers. You may also store **where you follow them** and **your handles** on those apps so they can message you there (Instagram, TikTok, etc.). Handles are Zone A, visible to that Influencer only, never used for matching, never logged to analytics. You can change the tier, pause, or disconnect in Settings. Disconnect hard-deletes the edge and the handles. The Influencer can query people who added them using only facts you opted to share at that tier, plus those handles. They cannot send you a 1:1 Bridger message. They may invite your Circle to an Event. **Not shipped yet.**
...
- **Influencer entitlement (planned):** a separate paid role from co-op membership. Paying unlocks the Influencer portal (query your Circle, create Circle events). Fans never pay to add an Influencer. Card checkout for this digital SKU is not offered inside the iOS app. **Not shipped yet.**
- **Event album storage add-on (planned):** a host may pay to raise one event's shared album cap. This is event-scoped, not personal unlimited storage. Unused quota dies with the album window. **Not shipped yet.**
...
|---|---|---|
| Camera | Add a photo or video to a Collage page (asked when you tap the shutter), video replies, profile photo, a photo on an Inside Joke (co-op), and scanning a friend's invite QR to add each other (asked when you open Scan a code). The scan reads only the QR's Bridger deep link; no image is kept. | Feature degrades; app still works. Scan a code shows a "turn on camera" / Open Settings prompt and you can still add friends by tapping their invite link. Inside Jokes still post as text. |
| Microphone | Video Updates, video replies, a voice note on a Collage page, recap voice answers (Friend Pod), Assistant voice questions (opt-in) | Same. Collage still works without voice notes |
| Photo library (read) | Profile photo; photos or short videos (≤20 s) you choose for a Collage page; one photo on an Inside Joke (co-op); (planned) photos you pick for an event album or a version-of-me quiz version. Asked only when you tap Upload or the camera-roll button. We read only the items you pick. | User can skip / use live capture instead. Inside Jokes still post as text. |
| Photo library (add only) | Save a quiz result card, a just-shot photo, or a finished collage page to your camera roll. Requested only when you tap Save; add-only, we never read your existing photos for this. | Skip; the page still lives in Bridger |
| Notifications | Alerts for friends, Touch Grass, events, etc. | In-app activity still works |
| Contacts | Optional: when you tap **Connect your contacts** on Friends (Add friend or the empty state), or **Connect contacts** / an invite slot during onboarding, or **Invite a friend** (demo-week access gate), Bridger first shows **Not now / Continue**, then asks permission to read contacts **on your device only**. On Friends, picking someone saves that one number on a private card you made and can text them an invite. Onboarding still has three invite slots ("Invite friends #1 / #2 / #3"). We never upload your address book. After contacts load (or after you send an invite), we may tell you we will notify you when friends join (when they redeem your link or QR, or when their phone matches a card you made). That alert uses the same in-app notification prefs as other connection alerts; it is not address-book matching. Counting an invite toward the co-op "invite 3 friends" progress means you opened SMS, completed the system share sheet, or (in a browser with no share sheet) copied the invite link from a slot, not that the friend joined yet. | Skip / Not now; you can still use a link, QR, or the three invite slots |
| Location (coarse) | Optional "where you met"; future **Local map** (friend radar) will also need coarse, opt-in sharing when that feature ships. Planned visit nudges: if a close friend travels near your city, we may ask *them* whether to tell you; only after they opt in would you get a "they're nearby / in your city" alert. Discover currently shows only a Coming soon teaser and does **not** request location for the map. | Skip; app works |
...
**Recap playback speed (on-device only):** the speed you pick for the weekly recap (1x up to 2.5x) is saved as a single number on this phone (AsyncStorage, `bridger.recap.speed`) so it stays your default for every listen and next week. It is never sent to our servers, never part of analytics as content (we may record the rate as a UI method, never who you listened to), and clearing the app's local storage removes it.
...
- **Matching / learning:** Nest scores FoF suggestions on opaque IDs using de-identified Zone B/C facts. Embeddings keep the meaning of those facts (words and quiz dial numbers) and do not convert that meaning into an explicit identity label first. Suggestion card “why” titles only cite Everyone+matchable shared facts. Private (`none`+matchable) quiz/personality signals may affect scores silently and never appear as evidence titles. Outcomes land in `matching_feedback` (features + label weights only; no message content, names, or UX-analytics). **Discoverable defaults off** for new accounts until you tap Get started on the Discover splash (master switch on `user_settings.discoverable`). Opting out of Discoverable or deleting an account purges suggestions, feedback pair-rows, and Zone C embeddings. Feedback snapshots age out (~18 months).
...
- **Founder-only ops agents (Grokbot and similar):** private tooling for the founder to discuss and optionally update Bridger's software (`FOUNDER-AGENTS.md`). **Not** a member product, **not** Billy, **not** accessible to other users. Must not be given production Zone A PII or secrets by default. If a future build wires Nest to an external model for this tool, document keys/health like any other outbound integration. Until then it is local/founder-session only and does not process other people's Bridger data.
...
- **Who chooses:** You choose what information enters a computation and what purpose that computation serves. Bridger chooses the engineering needed to make that authorized computation reliable and secure (Terms §7.8a).
- **Personal Vault and Personalize** (`PERSONAL-DATA.md`): your profile facts already live in Bridger's hosted vault (you own them; who-sees and matchable stay separate). Settings → Personalize is a preview only. It collects no new information and installs nothing. Future matchers may change how eligible people are ranked. They cannot widen who is eligible, or show a fact to someone you did not allow. We do not offer a "train models on my data" switch. Bring-your-own database and on-device-only storage are not built.
- **Machine learning goal** (same words as Terms §7.8a): Bridger may use machine learning on information you explicitly authorize for a feature. Private information is not used to train general-purpose AI models without separate permission. Wherever practical, Bridger uses models it operates itself so member information does not go to outside AI companies. Goal date: the first of September 2031, about 1 million accounts, or a co-op volunteer who builds and runs it. **TODO (legal):** confirm the goal is not an unintended warranty.
- **Where we are now:** matching scores already run on our servers. These still send feature material to an outside company, only for the feature you used: summaries, quiz help, module notes, and the optional assistant go to **Anthropic**; embeddings and speech-to-text go to **OpenAI**; web live captions may use the browser speech service (often Google or Apple) and are not logged. API calls use no-training / zero-retention terms. We do not fine-tune on user content. If we find another outbound AI path, we add it here. Code is flagged `COMPLIANCE GOAL (self-operated models)` until that call moves in-house.
...
- Matching is friends-of-friends + attribute overlap + quiz psychology + embeddings (Zone C). Friend-circle placements (Close / Friends / demote / block) may tune matching weights. Learning rows store six numbers and an outcome on opaque ids, never names, quiz answers, or messages. Embeddings rebuild from matchable facts when you finish a Personality quiz or edit a matchable profile field. Those vectors keep the meaning you shared (your words and quiz dial numbers). Bridger does not convert that meaning into an explicit identity label (a personality type, a belief category, or a condition name) in order to compare people.
...
- **Discover Personality quizzes** (Your Funny Bone, Your Vibe, What Gets You Going, The Friend Zone; in-app name, analytics still `connect_over`): when you finish one, Bridger stores only the scored 0–1 dimensions + confidence server-side (`quiz_results` + private `quiz.{slug}.*` attributes). Answers and explain text stay on your device / are never sent. These scores feed matching and the reveal "how you line up" percentages (in-app quiz title + % only). Hard-deleted with the account. **Behind the Scenes** is archived from the Discover list. If you already completed it, that owner-only data still follows the same rules (never on a profile, never shown to matches).
- **Connection reveal / In common:** after you and a new friend set how you met, Bridger compares what you already share at the circle each of you granted the other (one-way). We just met uses Acquaintance-visible facts (the Everyone layer). Friends / Close friends unlock more only on the side that granted that circle. Hobby follow-up answers (side by side), favorites items, music artists and picks, and quiz compatibility % can appear when both sides can see that fact. Raw quiz answers never cross. If nothing overlaps yet, the screens point you to Personality quizzes. Adding a friend is never blocked on a filled profile. Friend-of-friend suggestions on the last reveal screen only appear when you have Discover matching on; tapping Add sends a normal connect request (same Discover approvals path).

---

## 4 · How we use data

- Run the core app: friends, Updates, Events, Messages (limited), Discover, quizzes, Touch Grass, recap Friend Pod, co-op portal, and (planned) Circles / Influencer portal / version-of-me quizzes.
- Enforce tiers, blocks, and membership perks.
- Send in-app (and later push) notifications the user has allowed.
- **BeReal-like reminders** (`story_prompt`): optional. When you turn the toggle on (capture screen or Settings → Notifications), Bridger may send **1–3 notifications a day** at random times asking you to capture your life (BeReal-style), including **one mid-party nudge** when you are hosting or going to a live event (skipped if you already posted 3 updates that day). Party nudges open capture with the event pre-tagged so the photo can land in that event's album. Tapping opens the in-app capture screen. Off by default; turn off anytime in the same places. No one else sees that you enabled this.
...
- **Influencers you add to your Circle (planned):** they see profile facts at the visibility tier you picked, plus the handles you typed, so they can reach you off Bridger. They do not get a Bridger DM. You can disconnect anytime.
- **Service providers** that host or process data for us: Supabase (DB/Auth/Storage), **Twilio** (sign-in SMS only, not marketing), AWS (API host), PostHog (product analytics), AI providers (**Anthropic** for summaries, quiz help, module notes, and the optional assistant; **OpenAI** for embeddings and speech-to-text) **server-side only** through the PII firewall when that feature is used, **RevenueCat** (IAP entitlements), and **Stripe** (card membership Checkout / Customer Portal; card numbers stay with Stripe). Web live captions may use the browser's own speech service. Our goal is to replace the outside model calls with models we operate (Terms §7.8a). Public Privacy / Terms live at `https://bridger.social/privacy.html` and `https://bridger.social/terms.html`.
...
- Event album (planned): shared access expires 7 days after the event end unless that person saved the photo to their device. Personal Update retention is separate.
- Circle handles (planned): kept until you edit them, disconnect that Influencer, or delete your account.
- Recap answers: rolling 7-day window with lazy purge on playlist load. Co-op clips keep `expires_at` null. **This week's** Friend Pod is for everyone. **Earlier locked weeks** are only opened by co-op members, and only weeks that listener can hear (tier filter). Week labels and voice counts are not public totals. See `complete/RECAP-PODCAST.md`.
...
|---|---|
| 2026-09-21 | Sign-in SMS A2P disclosures on phone Sign in; Privacy/Terms SMS program wording (HELP/STOP, rates, no sale of SMS opt-in for marketing); public CTA at /sms-opt-in.html |
| 2026-09-21 | Personalize preview in Settings. Collects nothing. Future customization cannot bypass someone else's visibility or eligibility. No training switch. |
| 2026-09-21 | You choose what information enters a computation and what purpose it serves. Bridger chooses the engineering that makes that authorized computation reliable and secure. |
| 2026-09-21 | Machine learning goal: authorized features only, no general-model training without permission, self-operated models where practical (September 2031, about 1 million accounts, or a co-op volunteer). Today: Anthropic, OpenAI, and browser live captions are named. |
| 2026-09-21 | Matching embeddings keep your words and quiz dial numbers. They do not convert that meaning into an identity label before comparing people. |
| 2026-09-13 | Public Privacy / Terms at bridger.social/privacy.html and /terms.html. Twilio listed as the sign-in SMS processor. |
| 2026-09-13 | Optional Face ID / fingerprint lock over a saved session. We never store a face or print. Off by default. |
| 2026-09-13 | Sign-in phone OTP: country dial picker (device-region default). Analytics may record `country_iso` only, never digits. |
...
| 2026-09-09 | First-open CRT intro: after each typed screen we record whether Next was tapped or the five-second fill ran (`method` tap or auto, screen index only). Still no copy, no PII. |
| 2026-09-09 | Friend Pod self-locks each Monday (rose / thorn / bud + votes + fill-ins). Optional leftover-prompt model sees Bridger copy only. Suggested question text is stored for voting, never analytics. Recap share audience is Close / Friends / Acquaintances. |
| 2026-09-09 | New onboarding (preview) ends on the join / invite screen (same Apple Pay / Google Pay / card sheet as Old). Invite 3 still unlocks Free Lite. Congratulations splash is over Home, not a last onboarding page. |
| 2026-09-09 | New onboarding (preview): help-interest keys are feature ids; ⓘ notes are on-device copy; Friend Pod can show this week's questions and a suggested-question box (text is stored, never sent to analytics). |
| 2026-09-09 | Matching learns from Close / Friends placements (opaque feature snapshots). Quiz psychology (same vs opposite vs style chart) is in Nest scoring. Embeddings rebuild from matchable facts. No Edge Functions. |
| 2026-09-09 | Discover: Behind the Scenes archived from the live quiz list (data rules unchanged if already taken). User-facing name Connect Over → Personality quizzes. Reveal / In common empty states point there. Overlap after connect is one-way per granted circle. Adding a friend is never gated on quizzes. |
| 2026-09-09 | Friends: Connect your contacts on Add friend / empty state; one picked number saved on `pending_people`; notes on `pending/[id]` survive merge when they join with that number. Still never upload the address book. |
| 2026-09-09 | Circles (planned): separate Influencer edge, fan-chosen visibility tier, off-app handles, no 1:1 Bridger DM, portal query of opted-in facts. Event host notes + shared album save/7-day expiry/event-scoped quota + host add-on. Version-of-me user quizzes (UGC, no matching). Capture-only exceptions listed. |
| 2026-09-09 | Inside Jokes: full-screen composer; optional co-op photo (camera or one roll pick); color + tagged friend/event stored. Joke text never goes to analytics or AI. |
| 2026-09-09 | User-facing name for Updates pages: Scrapbook → **Collage** (tile, tab, copy). Code / tables stay `stories` / `scrapbook_*`. |
| 2026-09-09 | Collage camera-first: voice notes + optional server transcript; friend tags (`collage_tag`); save photo/page to camera roll; on-device subject lift (Vision / Android ML Kit) when available. Photos still never sent to AI. |
| 2026-09-08 | Phone OTP sign-in (E.164 on user_contacts); pending_people author-only cards merge on signup; New onboarding preference chips (membership / help / page authoring); Groups language for birthday audience |
...
| 2026-08-30 | J-name friend compatibility %: when both people finished, leaderboard shows a fun pairwise score from J-name + % + top picks only (no answers) |
...
| 2026-09-08 | Collages (then called Scrapbooks): Updates become 8.5 x 11 pages (elements + positions stored); camera-roll import allowed for pages (permission in context, only picked items read); Only me audience; 4 photos/videos a day across 1 to 4 pages; same-day edits bump a revision; draft stays on device. Apple privacy manifest now declares Photos or Videos as collected (app functionality, linked, not tracking) |
| 2026-09-10 | Founder-only ops agents (Grokbot): private tooling for the founder to discuss/update software; not Billy; not accessible to members; no production PII/secrets by default (`FOUNDER-AGENTS.md`) |
| 2026-09-09 | J-name: first finish is the durable server result (friends, compatibility, Profile, share link). Fun retakes stay on this phone only and never overwrite matching. |
| 2026-09-09 | J-name guest take: a friend can finish the quiz with no account (result on-device only). Making an account uploads that first result and adds the sharer as a friend so both can see the duo %. |
| 2026-09-08 | J-name quiz: mid-take progress may stay on this phone only (AsyncStorage, ~7 days) so leaving does not force a full retake; never uploaded; cleared on finish or retake |
| 2026-09-09 | Places onboarding: per-field Private / Close friends only toggles (DB `none` / `close`); wider sharing later via Privacy settings |
| 2026-09-09 | Discover Connect Over quizzes: on-device mid-take drafts (~14 days) with Save and exit / Discard; Local map teaser notes planned opt-in city-visit nudges (`friend_nearby` / `friend_city_visit` kinds reserved) |
| 2026-09-08 | Onboarding contacts: **Not now / Continue** alert before the OS contacts sheet so people can back out without getting stuck on Select Contacts / Allow Full Access |
| 2026-08-29 | Onboarding contacts: success turns green with a check + "AWESOME! We'll notify you when friends join."; invite slots do the same when sent |
