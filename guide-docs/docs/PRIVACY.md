...
- **Pending people (private cards):** if you invite or save a note about a specific contact, we may store that one phone (E.164) on an author-only `pending_people` row so we can merge your notes when they join. We never upload your whole address book. **Who sees it:** only you. **How to stop:** delete the note / account. When they join with that number, the card is replaced by their real profile; they do not see your notes.
...
- **Onboarding preference chips:** New onboarding may save opaque keys for membership interests, what would help, and how you like scrapbook pages made (`user_settings.membership_interests` / `help_interests` / `page_authoring`). These are preferences, never free-text or names.
...
- **Profile photo "looks" (filters):** you can style your profile photo with a look (**Pop art**, **Comic**, **X-ray**, or **Sepia**). All four looks are **baked on our own server** for the saved avatar: the phone uploads your normal photo to your private `media` bucket, then `POST /photo-filters/apply` has the NestJS API read that photo, repaint it with **ImageMagick** (a local image tool, not an outside company and not any AI model), and save the finished picture back as a new owned `media` row. The original stays on file (`user_identity.avatar_original_media_id`); your avatar points at the filtered copy so every face in the app renders that look fast. Which look you picked is stored as `user_identity.avatar_filter`. **Pop art** also shows an instant on-device Warhol-grid preview while the single-tile server bake finishes (a circular avatar cannot show a 4-tile grid cleanly). You can change the look later from Profile **Edit** → Photo look (re-bakes from the original; no re-upload required). **Why:** those looks need image operations that should not re-run on every Avatar render. **Who sees it:** only you until you tap Continue (or Save look), then it becomes your avatar under the normal tier rules. The photo is never sent to any AI/model or third party for this; the work happens inside our own container. Both the original and the filtered copy are hard-deleted with the account.
...
- **Updates, shown as Scrapbook pages** (2026-09-08). A post is one 8.5 x 11 page: up to 4 photos/videos a day across 1 to 4 pages, a typed caption, and a small date stamp, each stored as an "element" with its position (fractions of the page) in `scrapbook_pages` / `scrapbook_elements`. **Media may be captured live in Bridger or chosen from the camera roll** (the roll is read only for the pictures you pick; we never scan your library). Each photo remembers where it came from (`bridger_camera` / `camera_roll` / `event` / `shared`). A flattened preview image of the page is stored alongside so the Home tile and Catch-Up can show it. Pages can be edited the same day (a `revision` number goes up; friends see the ring light again on their own phone; no view counts). Audience per page: **Only me** (nobody else can read it, enforced by row-level security), Close, Friends, Everyone, or a co-op group. Same retention as before: ~24 h live, ~30 days for free members, forever for co-op; hard-deleted with the account. Photos are never sent to AI; the day summary still uses caption words only. Circle video replies and stickers stay capture-only. The unposted draft lives only on your phone and is cleared when you post.
...
- **Quiz result + share/referral (Which "J" name are you?):** `jname_results` stores your fun result only (a J-name, a percent, and your top J-name picks by score). A `jname_shares` row is one stable share link per person, snapshotting the J-name/percent so the free web page can render it. `jname_referrals` records that someone opened your link so that, if they later make an account, we can connect them to you ("who invited whom"). While a viewer is logged out we keep only an opaque device id (`anon_ref`), never a name. The public web view (`/q/<token>`) needs no account; the "your version of X" friend board requires an account (API `GET /jname/leaderboard`). When both you and a friend have finished, we also show a fun pairwise **compatibility %** derived only from those fun results (same J-name scores high; a top-pick hit scores medium-high; otherwise closeness of the two J-% numbers). Never answers or explanations. Notifications: `jname_link_opened` when someone opens your link; `jname_top_match` when a friend lands on a J-name in your top 3 picks. All of these hard-delete with the account.
...
| Contacts | Optional: when you tap **Connect contacts** (onboarding) or **Invite a friend** (demo-week access gate), Bridger first shows **Not now / Continue**, then asks permission to read contacts **on your device only** so you can pick someone to text your invite link. Onboarding also offers three separate invite slots ("Invite friends #1 / #2 / #3"). We never upload your address book. After contacts load (or after you send an invite), we celebrate on-screen and tell you **AWESOME! We'll notify you when friends join** (when they redeem your link or QR and become connected). That alert uses the same in-app notification prefs as other connection alerts; it is not address-book matching. Counting an invite toward the co-op "invite 3 friends" progress means you opened SMS, completed the system share sheet, or (in a browser with no share sheet) copied the invite link from a slot, not that the friend joined yet. | Skip / Not now; you can still use the three invite slots / system share sheet |
...
- **Discover Connect Over quizzes** (Your Funny Bone, Your Vibe, What Gets You Going, The Friend Zone): when you finish one, Bridger stores only the scored 0–1 dimensions + confidence server-side (`quiz_results` + private `quiz.{slug}.*` attributes). Answers and explain text stay on your device / are never sent. These scores feed matching and the reveal "how you line up" percentages (in-app quiz title + % only). Hard-deleted with the account.
- **Connection reveal / In common:** after you and a new friend set how you met, Bridger compares what you already share at the granted tier: hobby follow-up answers (side by side), favorites items, music artists and picks, and quiz compatibility %. Raw quiz answers never cross. Friend-of-friend suggestions on the last reveal screen only appear when you have Discover matching on; tapping Add sends a normal connect request (same Discover approvals path).
