...
/** The essential layer, one ask per screen. Only welcome-in completes it. */
...
}: {initialStep?: OnboardingStepKey;onComplete?: () => void;}) {
  const [step, setStep] = React.useState<OnboardingStepKey>(initialStep);
  const next = () => {
    const i = ORDER.indexOf(step);
    if (i < ORDER.length - 1) setStep(ORDER[i + 1]);else
    onComplete?.();
  };
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div
        ...
        {step === 'coop' && <CoopScreen onJoin={next} onSkip={next} />}
