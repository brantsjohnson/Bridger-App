// Bridger v1 cost model.
//
// The first version of Bridger is self-hosted (VPS + PostgreSQL + Caddy +
// Express + React + Mapbox + a domain). It is intentionally cheap and
// conditional, so this model is built around the costs that actually exist
// today and treats optional lines (email/SMS, push, object storage, payments,
// paid people) as toggles — email and push default on with low per-member
// estimates when amount is left at $0.
//
// Note on naming: we never say "AI" in copy. The only place a model would be
// used is friend/connection matching, so it is always called "matching".
...
export const DEFAULT_ANNUAL_DUES = 30
